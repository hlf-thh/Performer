...
def train_one_epoch(model, optimizer, data_loader, device, epoch, use_amp=False, warmup=True):
    ...
    lr_scheduler = None
    if epoch <= 5 and warmup is True:  # 当训练第一轮（epoch=0）时，启用warmup训练方式，可理解为热身训练
    ...
    # 在进程0中打印训练进度
    if is_main_process():
        data_loader = tqdm(data_loader, file=sys.stdout, colour='blue', ncols=100)
    ...
    return accu_loss.item() / (step + 1), train_acc
...
def evaluate(model, data_loader, device):
    ...
    # 在进程0中打印验证进度
    if is_main_process():
        data_loader = tqdm(data_loader, file=sys.stdout, colour='green', ncols=70)
